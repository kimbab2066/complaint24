<script setup>
import { ref, onMounted, computed, defineProps, watch } from 'vue'; // watch import 추가
...
const fetchSurveyDetail = async (surveyNo) => {
  loading.value = true;
  error.value = null; // 새로운 데이터 로딩 시 에러 초기화
  surveyDetail.value = null; // 새로운 데이터 로딩 시 기존 상세 정보 초기화
  questions.value = []; // 질문 목록 초기화
...
// 컴포넌트 마운트 시 실행될 로직
onMounted(() => {
  fetchSurveyDetail(props.survey_no);
});

// props.survey_no 변경을 감지하고 데이터 다시 불러오기
watch(
  () => props.survey_no,
  (newSurveyNo, oldSurveyNo) => {
    if (newSurveyNo !== oldSurveyNo) {
      fetchSurveyDetail(newSurveyNo);
    }
  },
...
const updateSurvey = async () => {
  // 라우트 파라미터 대신 props에서 survey_no를 가져옵니다.
  const surveyNo = props.survey_no;
  if (!surveyNo) {
  ...
  try {
    ...
    alert('내용이 수정되었습니다.');
    // 컴포넌트 내부 렌더링이므로 라우팅 대신 부모 컴포넌트에 이벤트를 전달할 수 있습니다.
    // router.push({ name: 'umy' }); // 제거 또는 주석 처리
  } catch (err) {
