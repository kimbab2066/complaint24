...
router.get("/", async (req, res) => {
  ...
  try {
    const rows = await query("inquiry"); // inquiry 전체 목록
    res.json(rows);
  } catch (err) {
    console.error("DB 조회 실패:", err);
    res.status(500).json({ message: "서버 오류" });
...
router.post("/", async (req, res) => {
  ...
  // 프론트에서 보내는 데이터 (RegistSurveyForm)
  const { surveyName, status, notice_no, questionList, writer } = req.body;
  ...
  try {
    ...
    if (!newInquiryNo) {
      throw new Error("조사지 생성 실패 (inquiry_no 없음)");
    }
    ...
    for (const q of questionList) {
      await conn.query(sql.questionInsert, [
        q.content, // 질문 내용
        q.responseType?.code || 1, // 답변 유형
        q.required ? 1 : 0, // 필수 여부
        newInquiryNo, // 외래키
        q.priority?.name || null, // 우선순위
      ]);
    }
...
=========================== */
router.get("/notices/list", async (req, res) => {
  console.log("[surveyRouter] GET /notices/list 요청 받음");

  try {
    const rows = await query("noticeList");

    res.status(200).json({
      notices: rows,
    });
  } catch (err) {
    console.error("Notice 목록 조회 실패:", err);
    res.status(500).json({ message: "서버 오류" });
  }
});
...
router.get("/detail/:id", async (req, res) => {
  ...
  console.log(`[surveyRouter] GET /detail/${id} 요청`);
  ...
  try {
    ...
    const basicInfoRows = await conn.query(sql.inquiryOrderBy, [id]);

    if (basicInfoRows.length === 0) {
      return res.status(404).json({ message: "조사지를 찾을 수 없습니다." });
    }

    const questionRows = await conn.query(sql.inquiryListOrderBy, [id]);
    ...
    console.error("[surveyRouter] 상세조회 오류:", error);
    res.status(500).json({ message: "서버 오류" });
  } finally {
    if (conn) conn.release();
  }
