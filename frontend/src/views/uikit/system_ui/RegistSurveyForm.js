...
export function RegistSurveyForm() {
  ...
  const basicInfo = ref({
    surveyName: '',
    businessItem: null, // 선택된 notice_no 와 연결됨
    status: null,
  });
  ...
  const fetchBusinessItems = async () => {
    try {
      ...
      businessItems.value = fetchedData.map((notice) => ({
        name: notice.content,
        code: notice.notice_no,
      }));
    } catch (error) {
      console.error('사업명 목록 로드 실패:', error);
      toast.add({
        severity: 'error',
        summary: '로딩 실패',
        detail: '공지사항 목록을 불러오지 못했습니다.',
  ...
  // 🔥 (2) 선택된 notice의 content 자동 업데이트 (제일 중요한 부분)
  watch(
    () => basicInfo.value.businessItem,
    (newVal) => {
      if (!newVal) {
      ...
      const selected = allNotices.value.find((notice) => notice.notice_no === newVal.code);

      relatedNoticeContent.value = selected ? selected.content : '';
    }
  );
  ...
  const clearTemp = () => {
    confirm.require({
      ...
      accept: () => {
        localStorage.removeItem(tempStorageKey);
        toast.add({ severity: 'info', summary: '삭제 완료', detail: '임시저장이 삭제되었습니다.' });
        window.location.reload();
      },
    });
  };

  // 🔥 (5) 최종 등록 — notice_no 추가됨
  const requestApproval = async () => {
    if (!basicInfo.value.surveyName)
      return toast.add({
        severity: 'warn',
    ...
    if (!basicInfo.value.businessItem)
      return toast.add({ severity: 'warn', summary: '입력 필요', detail: '사업명을 선택하세요.' });

    if (!basicInfo.value.status)
      return toast.add({ severity: 'warn', summary: '입력 필요', detail: '상태를 선택하세요.' });

    if (!questionList.value[0]?.content)
      return toast.add({ severity: 'warn', summary: '입력 필요', detail: '질문을 입력하세요.' });

    // 🔥 여기서 notice_no 포함
    const surveyData = {
      surveyName: basicInfo.value.surveyName,
      status: basicInfo.value.status.code,
      notice_no: basicInfo.value.businessItem.code, // ★ 추가됨
      questionList: questionList.value,
      writer: '관리자',
    };

    try {
      await axios.post('/api/system/survey', surveyData);
  ...
  // 🔥 (6) onMounted — 임시 저장된 데이터 복원
  onMounted(() => {
    fetchBusinessItems();

    const saved = localStorage.getItem(tempStorageKey);
    if (saved) {
      const parsed = JSON.parse(saved);
      basicInfo.value = parsed.basicInfo;
      questionList.value = parsed.questionList;
